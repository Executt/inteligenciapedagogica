    ...
    <AppShell>
      <div className="p-8 max-w-[1400px] mx-auto">
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <SkeletonTable rows={6} cols={3} /> : isError ? (
              <ErrorState description="Tente novamente em alguns instantes." />
            ) : (data?.length ?? 0) === 0 ? (
              <EmptyState title="Nenhum aluno encontrado" description="Cadastre alunos ou importe dados governamentais para começar." />
            ) : (
              <div className="divide-y divide-border">
                {data!.map((a) => (
                  <Link key={a.id} to="/aluno/$id" params={{ id: a.id }} className="flex items-center gap-4 py-3 hover:bg-accent/50 -mx-2 px-2 rounded transition-colors">
                    ...
                    <Badge variant={a.risco === "alto" ? "destructive" : a.risco === "medio" ? "warning" : "success"}>Risco {a.risco}</Badge>
                    <span aria-hidden="true" className="text-muted-foreground"><ChevronRight className="h-4 w-4" /></span>
                  </Link>
                ))}
              </div>
            )}
